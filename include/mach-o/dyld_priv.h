...
#define __ptrauth_dyld_objc_notify_init2                                       \
  __ptrauth(ptrauth_key_process_dependent_code, 1,                             \
            ptrauth_string_discriminator("_dyld_objc_notify_init3"))
#define __ptrauth_dyld_objc_notify_mapped3                                     \
  __ptrauth(ptrauth_key_process_dependent_code, 1,                             \
            ptrauth_string_discriminator("_dyld_objc_notify_mapped4"))
#define __ptrauth_dyld_objc_notify_unmapped                                    \
  __ptrauth(ptrauth_key_process_dependent_code, 1,                             \
            ptrauth_string_discriminator("_dyld_objc_notify_unmapped2"))
#define __ptrauth_dyld_dyld_objc_notify_patch_class                            \
  __ptrauth(ptrauth_key_process_dependent_code, 1,                             \
            ptrauth_string_discriminator("_dyld_objc_notify_patch_class2"))
#else
#define __ptrauth_dyld_objc_notify_init2
#define __ptrauth_dyld_objc_notify_mapped3
#define __ptrauth_dyld_objc_notify_unmapped
...
// Note that all changes to the order, including adding new entries, should
// involve bumping the value in SectionLocations::version Always add new entries
// to the end.  We can't reorder existing entries, as they are used in the open
// source drop of the swift runtime
...
typedef void (*_dyld_objc_notify_mapped)(unsigned count,
                                         ...
                                         const struct mach_header *const mh[]);
typedef void (*_dyld_objc_notify_init)(const char *path,
                                       const struct mach_header *mh);
typedef void (*_dyld_objc_notify_unmapped)(const char *path,
                                           const struct mach_header *mh);
typedef void (*_dyld_objc_notify_patch_class)(
    const struct mach_header *originalMH, void *originalClass,
...
struct _dyld_objc_notify_mapped_info {
  ...
  uint32_t dyldObjCRefsOptimized : 1, flags : 31;
};
typedef void (*_dyld_objc_notify_mapped2)(
    unsigned count, const struct _dyld_objc_notify_mapped_info infos[]);
typedef void (*_dyld_objc_notify_init2)(
    const struct _dyld_objc_notify_mapped_info *info);

typedef void (^_dyld_objc_mark_image_mutable)(uint32_t objcImageIndex);
typedef void (*_dyld_objc_notify_mapped3)(
    unsigned count, const struct _dyld_objc_notify_mapped_info infos[],
...
// initialized. Dyld will call back the "mapped" function with an array of
// images that contain an objc-image-info section. Those images that are dylibs
// will have the ref-counts automatically bumped, so objc will no longer need to
// call dlopen() on them to keep them from being unloaded.  During the call to
// _dyld_objc_notify_register(), dyld will call the "mapped" function with
// already loaded objc images.  During any later dlopen() call, dyld will also
// call the "mapped" function.  Dyld will call the "init" function when dyld
// would be called initializers in that image.  This is when objc calls any
// +load methods in that image.
//
...
struct _dyld_objc_callbacks_v4 {
  ...
  __ptrauth_dyld_dyld_objc_notify_patch_class _dyld_objc_notify_patch_class
      patches;
};
...
//	info->mh							mach
//header of image containing addr
//  info->dwarf_section					pointer to start of
//  __TEXT/__eh_frame section info->dwarf_section_length
//  length of __TEXT/__eh_frame section info->compact_unwind_section
//  pointer to start of __TEXT/__unwind_info section
...
extern const struct mach_header *
...
extern dyld_platform_t dyld_get_active_platform(void)
    __API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0));

// Base platforms are platforms that have version numbers (macOS, iOS, watchos,
// tvOS, bridgeOS) All other platforms are mapped to a base platform for version
// checks

// It is intended that most code in the OS will use the version set constants,
// which will correctly deal with secret and future platforms. For example:

//  if (dyld_program_sdk_at_least(dyld_fall_2018_os_versions)) {
//      New behaviour for programs built against the iOS 12, tvOS 12, watchOS 5,
//      macOS 10.14, or bridgeOS 3 (or newer) SDKs
//  } else {
//      Old behaviour
//  }

// In cases where more precise control is required (such as APIs that were added
// to various platforms in different years) the os specific values may be used
// instead. Unlike the version set constants, the platform specific ones will
// only ever return true if the running binary is the platform being tested,
// allowing conditions to be built for specific platforms and releases that came
// out at different times. For example:
...
extern bool dyld_sdk_at_least(const struct mach_header *mh,
                              dyld_build_version_t version)
    __API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0));

// Takes a version and returns if the image was built with that minos version or
// newer In the case of multi_plaform mach-o's it tests against the active
// platform
extern bool dyld_minos_at_least(const struct mach_header *mh,
                                dyld_build_version_t version)
    __API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0));

// Convenience versions of the previous two functions that run against the the
// main executable
...
// Returns a token that can be used for version compares. This is intend to be
// used by frameworks that must send the information over to a daemon which
// performs the compare. These values should not be stored and are not
// guaranteed to be stable between OS releases.

extern uint64_t dyld_get_program_sdk_version_token()
    __SPI_AVAILABLE(macos(15.0), ios(18.0), watchos(11.0), tvos(18.0),
                    bridgeos(9.0));
extern uint64_t dyld_get_program_minos_version_token()
    __SPI_AVAILABLE(macos(15.0), ios(18.0), watchos(11.0), tvos(18.0),
                    bridgeos(9.0));
extern dyld_platform_t dyld_version_token_get_platform(uint64_t token)
    __SPI_AVAILABLE(macos(15.0), ios(18.0), watchos(11.0), tvos(18.0),
                    bridgeos(9.0));
extern bool dyld_version_token_at_least(uint64_t token,
                                        dyld_build_version_t version)
    __SPI_AVAILABLE(macos(15.0), ios(18.0), watchos(11.0), tvos(18.0),
                    bridgeos(9.0));

// Function that walks through the load commands and calls the internal block
// for every version found Intended as a fallback for very complex (and rare)
// version checks, or for tools that need to print our everything for diagnostic
// reasons
extern void dyld_get_image_versions(const struct mach_header *mh,
                                    void (^callback)(dyld_platform_t platform,
...
// Because we now have so many different OSes with different versions these
// version set values are intended to to provide a more convenient way to
// version check. They may be used instead of platform specific version in
// dyld_sdk_at_least(), dyld_minos_at_least(), dyld_program_sdk_at_least(), and
// dyld_program_minos_at_least(). Since they are references into a lookup table
// they MUST NOT be used by any code that does not ship as part of the OS, as
// the values may change and the tables in older OSes may not have the necessary
// values for back deployed binaries. These values are future proof against new
// platforms being added, and any checks against platforms that did not exist at
// the epoch of a version set will return true since all versions of that
// platform are inherently newer.
...
//
// Note on watchOS and bridgeOS, this returns the equivalent iOS SDK version
// number (i.e an app built against watchOS 2.0 SDK returns 9.0).  To see the
...
#if TARGET_OS_WATCH
// watchOS only.
// This finds the Watch OS SDK version that the main executable was built
// against. Exists in Watch OS 2.0 and later
extern uint32_t dyld_get_program_sdk_watch_os_version(void)
    __API_AVAILABLE(watchos(2.0));

// watchOS only.
// This finds the Watch min OS version that the main executable was built to run
// on. Note: dyld_get_program_min_os_version() returns the iOS equivalent
// (e.g. 9.0)
...
// NOTE: It is safe to call this prior to malloc being initialized.  This
// function is guaranteed to not call malloc, or depend on its state.
//
// Exists in Mac OS X 10.9 and later
extern bool dyld_process_is_restricted(void);

//
// Returns path used by dyld for standard dyld shared cache file for the current
// arch.
//
// Exists in Mac OS X 10.11 and later
...
//
// Returns if there are any inserted (via DYLD_INSERT_LIBRARIES) or interposing
// libraries.
//
...
//
// Return true if dyld contains a fix for a specific identifier. Intended for
// staging breaking SPI changes
//
...
//
// <rdar://problem/13820686> for OpenGL to tell dyld it is ok to deallocate a
// memory based image when done.
//
// Exists in Mac OS X 10.9 and later
...
// NOTE: this is less safe than using static interposing via
// DYLD_INSERT_LIBRARIES because the running program may have already copy the
// pointer values to other locations that dyld does not know about.
//
...
#ifdef __BLOCKS__
//
// Given the UUID of a dyld shared cache file, this function will attempt to
// locate the cache file and if found iterate all images, returning info about
// each one.  Returns 0 on success.
//
// Exists in Mac OS X 10.11 and later
//           iOS 9.0 and later
...
//
// Given the UUID of a dyld shared cache file, and a NULL terminated array of
// extra directory paths to search, this function will scan the standard and
// extra directories looking for a cache file that matches the UUID and if found
// iterate all images, returning info about each one.  Returns 0 on success.
//
// Exists in Mac OS X 10.12 and later
//           iOS 10.0 and later
extern int dyld_shared_cache_find_iterate_text(
    const uuid_t cacheUuid, const char *extraSearchDirs[],
    void (^callback)(const dyld_shared_cache_dylib_text_info *info));
#endif /* __BLOCKS */
...
//
// Returns the start address of the dyld cache in the process and sets length to
// the size of the cache. Returns NULL if the process is not using a dyld shared
// cache
//
...
// Exists in Mac OS X 10.15 and later
// Exists in iOS 13.0 and later
extern bool dyld_need_closure(const char *execPath,
...
extern void _dyld_images_for_addresses(unsigned count, const void *addresses[],
                                       struct dyld_image_uuid_offset infos[]);

//
// Lets you register a callback which is called each time an image is loaded and
// provides the mach_header*, path, and whether the image may be unloaded later.
// During the call to _dyld_register_for_image_loads(), the callback is called
// once for each image currently loaded.
//
// Exists in macOS 10.14 and later
// Exists in iOS 12.0 and later
extern void
_dyld_register_for_image_loads(void (*func)(const struct mach_header *mh,
                                            const char *path, bool unloadable));

//
// Lets you register a callback which is called for bulk notifications of images
// loaded. During the call to _dyld_register_for_bulk_image_loads(), the
// callback is called once with all images currently loaded. Then later during
// dlopen() the callback is called once with all newly images.
//
// Exists in macOS 10.15 and later
// Exists in iOS 13.0 and later
extern void _dyld_register_for_bulk_image_loads(void (*func)(
    unsigned imageCount, const struct mach_header *mhs[], const char *paths[]));

//
// DriverKit main executables do not have an LC_MAIN.  Instead
// DriverKit.framework's initializer calls _dyld_register_driverkit_main() with
// a function pointer that dyld should call into instead of using LC_MAIN.
//
extern void _dyld_register_driverkit_main(void (*mainFunc)(void));

//
// This is similar to _dyld_shared_cache_contains_path(), except that it returns
// the canonical shared cache path for the given path.
//
...
//
// Dyld has a number of modes. This function returns the mode for the current
// process. dyld2 is the classic "interpreter" way to run. dyld3 runs by
// compiling down and caching what dyld needs to do into a "closure".
//
// Exists in macOS 10.16 and later
// Exists in iOS 14.0 and later
//
#define DYLD_LAUNCH_MODE_USING_CLOSURE                                         \
  0x00000001 // dyld4: 0 => main is JITLoader, 1=> main is PrebuiltLoader
#define DYLD_LAUNCH_MODE_BUILT_CLOSURE_AT_LAUNCH                               \
  0x00000002 // dyld4: currently unused
#define DYLD_LAUNCH_MODE_CLOSURE_SAVED_TO_FILE                                 \
  0x00000004 // dyld4: built and wrote PrebuiltLoaderSet to disk
#define DYLD_LAUNCH_MODE_CLOSURE_FROM_OS                                       \
  0x00000008 // dyld4: PrebuiltLoaderSet used was built into dyld cache
#define DYLD_LAUNCH_MODE_MINIMAL_CLOSURE 0x00000010 // dyld4: unused
#define DYLD_LAUNCH_MODE_HAS_INTERPOSING                                       \
  0x00000020 // dyld4: process has interposed symbols
#define DYLD_LAUNCH_MODE_OPTIMIZED_DYLD_CACHE                                  \
  0x00000040 // dyld4: dyld shared cache is optimized (stubs eliminated)
...
//
// When it has more information about the termination, dyld will use
// abort_with_payload(). The payload is a dyld_abort_payload structure.  The
// fixed fields are offsets into the payload for the corresponding string.  If
// the offset is zero, that string is not available.
//
struct dyld_abort_payload {
  uint32_t version; // first version is 1
  uint32_t
      flags; // 0x00000001 means dyld terminated at launch, backtrace not useful
  uint32_t targetDylibPathOffset; // offset in payload of path string to dylib
                                  // that could not be loaded
  uint32_t clientPathOffset;      // offset in payload of path string to image
                                  // requesting dylib
  uint32_t symbolOffset; // offset in payload of symbol string that could not be
                         // found
                         // string data
};
...
// never called from source code. Used by static linker to implement lazy
// binding
...
// isLoaded is true if that class is in a binary which has been previously
// passed to the objc load notifier. Note you can set stop to true to stop
// iterating. Also note, this function must be called after
// _dyld_objc_notify_register.
//
// Exists in Mac OS X 10.15 and later
// Exists in iOS 13.0 and later
extern void _dyld_for_each_objc_class(const char *className,
                                      void (^callback)(void *classPtr,
                                                       bool isLoaded,
                                                       bool *stop));

// Called only by objc to see if dyld has pre-optimized protocols with this
// name. The callback will be called once for each protocol with the given name
// where isLoaded is true if that protocol is in a binary which has been
// previously passed to the objc load notifier. Note you can set stop to true to
// stop iterating. Also note, this function must be called after
// _dyld_objc_notify_register.
//
// Exists in Mac OS X 10.15 and later
// Exists in iOS 13.0 and later
extern void _dyld_for_each_objc_protocol(const char *protocolName,
                                         void (^callback)(void *protocolPtr,
                                                          bool isLoaded,
                                                          bool *stop));
...
// Called only by libobjc to get the number of classes in the shared cache hash
// table
//
// Exists in Mac OS X 12.0 and later
// Exists in iOS 15.0 and later
...
// Called only by libobjc to check if relative method lists are the new large
// caches format
//
// Exists in Mac OS X 12.0 and later
// Exists in iOS 15.0 and later
...
struct _dyld_protocol_conformance_result {
  ...
  // Contains a ProtocolConformanceDescriptor iff `kind` is
  // _dyld_protocol_conformance_result_kind_found_descriptor Contains a
  // WitnessTable iff `kind` is
  // _dyld_protocol_conformance_result_kind_found_witness_table
  const void *value;
};

// Called only by Swift to see if dyld has pre-optimized protocol conformances
// for the given protocolDescriptor/metadataType and typeDescriptor.
//
// Exists in Mac OS X 12.0 and later
// Exists in iOS 15.0 and later
...
// Called only by Swift to see if dyld has pre-optimized protocol conformances
// for the given foreign type descriptor name and protocol
//
// Exists in Mac OS X 12.0 and later
// Exists in iOS 15.0 and later
extern struct _dyld_protocol_conformance_result
...
// Called only by Swift to check what version of the optimizations are
// available.
//
// Exists in Mac OS X 12.0 and later
// Exists in iOS 15.0 and later
...
extern uint32_t _dyld_swift_optimizations_version(void)
    __API_AVAILABLE(macos(12.0), ios(15.0), watchos(8.0), tvos(15.0));

// Swift uses this define to guard for the above symbol being available at build
// time
#if !TARGET_OS_EXCLAVEKIT
...
// Called only by Swift to check if dyld has pre-optimized protocol conformances
// in the closure for the given on-disk mach_header containing a __swift5_proto
// section
//
// Exists in Mac OS X 13.0 and later
// Exists in iOS 16.0 and later
extern bool _dyld_has_preoptimized_swift_protocol_conformances(
    const struct mach_header *mh);

// Called only by Swift to see if dyld has pre-optimized protocol conformances
// for the given protocolDescriptor/metadataType and typeDescriptor in the
// closure on disk.
//
// Exists in Mac OS X 13.0 and later
// Exists in iOS 16.0 and later
...
// Called only by Swift to see if dyld has pre-optimized protocol conformances
// for the given foreign type descriptor name and protocol in the closure on
// disk.
//
// Exists in Mac OS X 13.0 and later
// Exists in iOS 16.0 and later
...
// Swift uses this define to guard for the above symbols being available at
// build time
#if !TARGET_OS_EXCLAVEKIT
...
typedef enum { dyld_objc_string_kind } DyldObjCConstantKind;

// CF constants such as CFString's can be moved in to a contiguous range of
// shared cache memory.  This returns true if the given pointer is to an object
// of the given kind.
//
// Exists in Mac OS X 10.16 and later
// Exists in iOS 14.0 and later
...
typedef char *(*_dyld_pseudodylib_lookup_symbols)(
    void *pd_ctx, const void *mh, const char *names[], size_t num_names,
    void *addrs[], _dyld_pseudodylib_symbol_flags flags[]);
typedef int (*_dyld_pseudodylib_lookup_address)(void *pd_ctx, const void *mh,
                                                const void *addr,
                                                struct dl_info *dl);
typedef char *(*_dyld_pseudodylib_find_unwind_sections)(
    void *pd_ctx, const void *mh, const void *addr, bool *found,
    struct dyld_unwind_sections *info);
typedef char *(*_dyld_pseudodylib_loadable_at_path)(void *pd_ctx,
                                                    const void *mh,
                                                    const char *possible_path);
typedef char *(*_dyld_pseudodylib_finalize_requested_symbols)(
    void *pd_ctx, const void *mh, const char *names[], size_t num_names);
...
// Deprecated pseudo-dylib v1 callbacks. See _dyld_pseudodylib_callbacks_v2
// below.
...
//
// dispose_error_message will be called to destroy error messages returned by
// the other callbacks. initialize will be called by dlopen to run initializers
// in the pseudo-dylib. deinitialize will be called by dlclose to run
// deinitializers. lookup_symbols will be called to find the address of symbols
// defined by the pseudo-dylib (e.g. by dlsym). lookup_address will be called by
// dladdr to find information about the given address. find_unwind_sections will
// be called by _dyld_find_unwind_sections. loadable_at_path will be called to
// check whether the pseudo-dylib should be loaded for a given candidate path.
...
//
// These APIs can be used to register "pseudo-dylibs" which present as dylibs
// when accessed via the dlfcn.h functions (dlopen, dlclose, dladdr, dlsym), but
// are backed by a set of callbacks rather than a full mach-o image.
//
// _dyld_pseudodylib_register_callbacks is used to register a set of callbacks
// that can be shared between multiple pseudo-dylibs. On success,
// _dyld_pseudodylib_register_callbacks will return a handle that can be used in
// calls to register pseudo-dylib instances (see _dyld_pseudodylib_register
// below). On failure it will return null. Registered callbacks should be
// deregistered by calling _dyld_pseudodylib_deregister_callbacks once all
// pseudo-dylibs using the callbacks have been deregistered.
//
// _dyld_pseudodylib_register is used to register an instance of a pseudo-dylib.
// This can be thought of as equivalent to creating a dylib on disk: the
// pseudo-dylib is not yet open, but can be found via its install-name by
// dlopen. Registration takes the address range that the pseudo-dylib can
// occupy, and this range must start with a valid mach header and load commands
// containing, at minimum, an LC_VERSION_MIN and LC_ID_DYLIB command identifying
// the pseudo-dylib's install name. The callbacks argument identifies the set of
// callbacks to use for this pseudo-dylib instance, and the opaque context
// pointer will be passed to each of these callbacks. Once a pseudo-dylib is no
// longer needed it should be deregistered by calling
// _dyld_pseudodylib_deregister (equivalent to "rm'ing" a dylib on disk).
//
// Exists in Mac OS X 14.0 and later
// Exists in iOS 17.0 and later
extern _dyld_pseudodylib_callbacks_handle _dyld_pseudodylib_register_callbacks(
    const struct _dyld_pseudodylib_callbacks *callbacks);
extern void _dyld_pseudodylib_deregister_callbacks(
    _dyld_pseudodylib_callbacks_handle callbacks_handle);
...
// dyld will patch those uses to DYLD_BADROOT_MARKER, or NULL if the use is a
// weak-import. That will cause a crash and the crash will be easy to identify
// in crash logs.
#define DYLD_BADROOT_MARKER 0xbad4007

// Called only by libobjc to check if dyld has loaded the image described by
// imageID, that contains pre-optimized categories The imageID parameter is a
// private interface between dyld and libobjc, and no assumption should be made
// about its value. Exists in Mac OS X 14.0 and later Exists in iOS 17.0 and
// later
extern bool _dyld_is_preoptimized_objc_image_loaded(uint16_t imageID);

// Called only by libobjc to access RW objc header information from the shared
// cache Exists in Mac OS X 14.0 and later Exists in iOS 17.0 and later
extern void *_dyld_for_objc_header_opt_rw();

// Called only by libobjc to access RO objc header information from the shared
// cache Exists in Mac OS X 14.0 and later Exists in iOS 17.0 and later
...
// Exists in iOS 18.0 and later
extern void
_dyld_register_dlsym_notifier(void (*callback)(const char *symbolName));

// Called only by Swift runtime to access top level symbol with prespecialized
// metadata.
extern const void *_dyld_get_swift_prespecialized_data(void)
...
// Returns true if the handle (returned by a prior call to dlopen()) points to a
// pseudo dylib Exists in macOS 15.0 and later Exists in iOS 18.0 and later
extern bool _dyld_is_pseudodylib(void *handle)
    __API_AVAILABLE(macos(15.0), ios(18.0), watchos(11.0), tvos(18.0));

// Called by Swift runtime to query pointer hash tables.
extern const void *
_dyld_find_pointer_hash_table_entry(const void *table, const void *key1,
...
// Returns the base address and size of each prewarming metadata range in the
// cache Exists in macOS 15.4 and later Exists in iOS 18.4 and later
extern void _dyld_for_each_prewarming_range(void (*callback)(const void *base,
                                                             size_t size));
